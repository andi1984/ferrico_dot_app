...
- **Downloads auto-sync** with GitHub Releases: a small client-side `fetch` of
  `/releases/latest` points each platform card at the matching asset (`.dmg` /
  `.AppImage` / `-setup.exe`) and stamps the version. No edits needed per
  release. If the request fails (offline / API rate limit), the cards fall back
  to the releases page. To re-map assets, edit the `PICK` matchers in the
  inline script.
